...
const VectorIndexLength: number = 256 * 256 * (4 + 4);
...
class Searcher {
  ...
  async [getStartEndPtr](
    ...
    if (this._vectorIndex) {
      ...
      const buf = (await this[getBuffer](256 + idx, 8, fd, ioStatus)) as Buffer;
  ...
  async search(ip: string) {
    ...
    if (!this._buffer) {
      fd = (await this[openFilePromise](this._dbFile) as number);
    }
    ...
    const { sPtr, ePtr } = (await this[getStartEndPtr](idx, fd, ioStatus)) as {
      sPtr: number;
      ePtr: number;
    };
    let l = 0;
...
const _checkFile = (dbPath: PathLike | null) => {
  try {
...
const newWithFileOnly = (dbPath: PathLike | null) => {
  _checkFile(dbPath);

  return new Searcher(dbPath, null, null);
};

const newWithVectorIndex = (
  dbPath: PathLike | null,
  vectorIndex: Buffer | null
) => {
  _checkFile(dbPath);
...
const newWithBuffer = (buffer: Buffer | null) => {
...
const loadVectorIndexFromFile = (dbPath: PathLike | null) => {
...
const loadContentFromFile = (dbPath: PathLike | null) => {
